...
BLOCK_OPEN_2_REWARD = 0.2
BLOCK_OPEN_3_REWARD = 0.4
...
def episode_shaping_reward(states, mark):
    ...
    - +block rewards when opponent open 2/3 lines decrease after the move.
    """
    total = 0.0
    opp_mark = " O " if mark == " X " else " X "
    ...
    for state, act_id in states:
        ...
        opp_open_2_before = count_open_sequences(before, opp_mark, 2)
        opp_open_2_after = count_open_sequences(after, opp_mark, 2)
        opp_open_3_before = count_open_sequences(before, opp_mark, 3)
        opp_open_3_after = count_open_sequences(after, opp_mark, 3)

        create_open_2 = max(0, own_open_2_after - own_open_2_before)
        create_open_3 = max(0, own_open_3_after - own_open_3_before)
        block_open_2 = max(0, opp_open_2_before - opp_open_2_after)
        block_open_3 = max(0, opp_open_3_before - opp_open_3_after)

        total += create_open_2 * CREATE_OPEN_2_REWARD
        total += create_open_3 * CREATE_OPEN_3_REWARD
        total += block_open_2 * BLOCK_OPEN_2_REWARD
        total += block_open_3 * BLOCK_OPEN_3_REWARD
